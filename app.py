...
# 역사적 인물 및 국가 간의 복잡한 권력 관계를 직관적인 네트워크 그래프로 시각화하기 위해 사용
from streamlit_agraph import agraph, Node, Edge, Config
import time

# 1. 학습 환경(웹페이지) 설정
# 목적: 학생들에게 디지털 학습 환경을 제공하고, 수업 주제를 명확히 인지시킴.
st.set_page_config(page_title="갑신정변 역사 탐구", layout="wide")

# --------------------------------------------------------------------------
# [자료 관리] 이미지 URL 데이터베이스
# 목적: 탭1과 탭3에 사용할 시각 자료를 중앙에서 관리해서 수정하기 용이하게 함.
# --------------------------------------------------------------------------
IMAGE_URLS = {
    # 탭 1 관계도용 아이콘
    ...
    # 탭 3 시뮬레이션용 각 날짜별 메인 이미지
    "Day1": "https://i.ibb.co/zW92HRsr/DAY1.png",
    "Day2": "https://i.ibb.co/Ld3J6X8V/DAY2.png",
    "Day3": "https://i.ibb.co/fdFSJbqD/DAY3.png"
}

# 탭 3 심화 학습용 추가 자료 이미지 (보충 자료가 필요한 학생들을 위함)
EXTRA_IMAGES = {
    "Day1": [
        "https://i.ibb.co/WNJG9M5B/day1p.png",   # 1일차 현장 기록화
        "https://i.ibb.co/BHjTBBbX/dat1map.png"  # 1일차 이동 경로 지도
    ],
    "Day2": [
        "https://i.ibb.co/99Rb3bnd/day2p.png",    # 2일차 기록화
        "https://i.ibb.co/BVBXtngJ/day2map.png"   # 2일차 주요 거점 지도
    ],
    "Day3": [
        "https://i.ibb.co/WQ9SmVK/day3p.png",     # 3일차 전투 기록화
        "https://i.ibb.co/6RvMx16x/day3map.png"   # 3일차 대치 지도
    ]
}

# --------------------------------------------------------------------------
# [기능 함수] 텍스트 타이핑 효과 (Streaming)
# 목적: 챗봇이 실시간으로 생각하며 대화하는 듯한 느낌을 부여하여,
#       학생들이 AI 역사 인물과의 상호작용에 더 깊이 몰입하도록 유도함.
# --------------------------------------------------------------------------
def stream_data(text):
    for char in text:
        yield char
        time.sleep(0.03) # 글자당 0.03초 딜레이를 주어 자연스러운 읽기 속도 유도

# --------------------------------------------------------------------------
# [학습 상태 관리] 세션 스테이트(Session State) 초기화
# 목적: Streamlit은 상호작용 시마다 코드가 재실행되므로, 학생의 학습 진행 상황
#       (현재 페이지, 퀴즈 정답 여부, 채팅 내역, 시뮬레이션 선택 값 등)이 초기화되지 않고 유지되도록 함.
# --------------------------------------------------------------------------

# 현재 학습 중인 페이지 단계 추적 (인트로 -> 탭1 -> 탭2 -> 탭3 -> 탭4)
if 'current_page' not in st.session_state:
    st.session_state['current_page'] = 'intro'

# 학생 이름 저장 (수업 내내 맞춤형 피드백 제공을 위함)
if 'student_name' not in st.session_state:
    st.session_state['student_name'] = ""

# [탭 1 상태] 퀴즈 단계 및 완료 여부
...
# [탭 2 상태] 롤플레잉 채팅 역할 및 진행 단계
if 'chat_role' not in st.session_state:
    st.session_state['chat_role'] = None # 학생이 선택한 역할 (김옥균/김홍집)
if 'chat_stage' not in st.session_state:
    st.session_state['chat_stage'] = 0 # 대화 진행 순서 (0:시작전, 1~3:대화중, 4:완료)
if 'chat_history' not in st.session_state:
    st.session_state['chat_history'] = [] # 대화 맥락 유지를 위한 히스토리 저장

# [탭 3 상태] 시뮬레이션 선택 및 결과 수치
if 'sim_metrics' not in st.session_state:
    # 초기값 설정: 성공확률 30%, 지지율 20%, 국방력 20% (실제 역사적 위기 상황을 수치로 반영)
...
# [탭 3 상태] 이미지 더보기 버튼 토글 (추가 자료 열기/닫기)
...
# --------------------------------------------------------------------------
# [페이지 0] 인트로: 학습 주제 안내 및 이름 입력
# 목적: 수업의 목표를 명확히 제시하고, 학생의 이름을 입력받아 수업 참여 동기를 유발함.
# --------------------------------------------------------------------------
def render_intro():
    ...
    with col2:
        ...
        # 이름을 입력해야만 다음 단계로 넘어갈 수 있도록 조건 설정 (유효성 검사)
        if st.button("수업 시작하기 🚀", type="primary"):
            if name_input.strip() != "":
                st.session_state['student_name'] = name_input
                st.session_state['current_page'] = 'tab1' # 탭 1로 화면 전환
                st.rerun()
            else:
                st.warning("이름을 입력해야 시작할 수 있어요!")

# --------------------------------------------------------------------------
# [페이지 1] 탭 1: 정치 마당 (퀴즈 + 관계도 시각화)
# 목적: 텍스트로는 파악하기 힘든 복잡한 권력 관계를 퀴즈를 통해 학습하고,
#       퀴즈 완료 후 네트워크 그래프로 시각화하여 구조적으로 이해하게 함 (CT: 추상화, 패턴인식).
# --------------------------------------------------------------------------
def render_tab1():
    # 개인화된 제목 출력 (학생 이름 포함)
    ...
    # [좌측] 퀴즈 영역
    with col_quiz:
        # 단계별 퀴즈 데이터 (문제, 보기, 정답, 해설) - 내용 지식(CK) 확인용
        quiz_data = {
            ...
            with st.form(key=f"quiz_form_{st.session_state['step']}"):
                ...
                if submit_btn:
                    if choice == current_q["correct"]:
                        ...
                        st.session_state['show_next'] = True # 다음 문제 버튼 활성화
                    else:
                        # 오답 시 안내 문구 제공 (즉각적 피드백으로 학습 유도)
                        st.error("❌ 오답입니다. 다시 한번 생각해서 옳은 답을 골라보세요!")
                        st.warning(f"힌트: {current_q['expl']}")

            # 정답을 맞췄을 때만 '다음 문제' 버튼 표시
            if st.session_state['show_next']:
                if st.session_state['step'] < 5:
                    if st.button("다음 문제 ➡️"):
                        st.session_state['step'] += 1
                        st.session_state['show_next'] = False
                        st.rerun() # 화면 갱신하여 다음 문제 로드
                else:
                    # 마지막 문제 정답 시 결과 보기 버튼 표시
    ...
    # [우측] 시각화 영역 (Network Graph)
    with col_viz:
        st.subheader("📊 권력 관계 시각화")
        
        # 퀴즈 완료 후에만 그래프를 보여주어 학습 보상으로 제공
        if st.session_state['quiz_finished']:
            # 노드(Node): 인물/국가 설정 (크기(size)로 당시 권력의 크기를 직관적으로 표현)
            nodes = [
            ...
            # 엣지(Edge): 관계 설정 (색상과 선 스타일로 협력/대립 관계의 성격 표현)
            edges = [
            ...
            # 물리 엔진(physics) 적용하여 노드들이 서로 밀어내며 자리 잡는 동적 효과 구현
            ...
            # 퀴즈를 풀기 전에는 그래프를 숨겨두어(잠금 이미지) 학습 동기 유발
...
# 목적: 학생이 직접 역사 속 인물(김옥균/김홍집)이 되어 상대방과 대화하며,
#       당시의 정치적 입장(급진 vs 온건)을 역지사지로 이해하게 함 (PK: 감정이입 학습).
# --------------------------------------------------------------------------
def render_tab2():
    ...
    if st.session_state['chat_role'] is None:
        ...
        with col1:
            ...
            if st.button("나는 '김옥균' (급진개화파)"):
                ...
                st.session_state['chat_history'] = [] # 대화 기록 초기화 (새로운 대화 시작)
                st.rerun()
        with col2:
    ...
    # 2. 채팅 인터페이스 (역할 선택 후)
        ...
        # [첫 인사말 처리] 기록이 없으면 챗봇이 먼저 말을 걸도록 설정하여 대화의 물꼬를 틈
        if not st.session_state['chat_history']:
            ...
            # 기존 대화 기록이 있으면 순서대로 표시 (중복 출력 방지)
        ...
        # 사용자 입력 처리 (학생이 직접 답변을 고민하고 작성하게 함)
        if prompt := st.chat_input("답변을 입력하세요..."):
            st.session_state['chat_history'].append({"role": "user", "content": prompt})
            with st.chat_message("user"):
                st.write(prompt)

            # 챗봇 시나리오 로직 (Rule-based)
            # 학생의 답변에 특정 키워드가 포함되었는지 확인하여, 역사적 맥락(CK)에 맞는 정교한 피드백 제공
            ...
            if my_role == 'Kim_Ok':
                if stage == 1:
                    ...
                    # '끊', '자주' 등 자주적 입장의 키워드가 있으면 정답으로 인정
        ...
        # 미션 완료 시 다음 단계로 넘어가는 버튼 표시
        if st.session_state['chat_stage'] == 4:
            st.success("🎉 대화 미션 완료!")
            col_btn1, col_btn2 = st.columns(2)
            with col_btn1:
                # '다시 채팅하기' 버튼: 다른 입장도 체험해볼 수 있도록 모든 상태를 초기화하여 초기 화면으로 돌아감
...
# 목적: 역사적 사건의 분기점마다 학생이 직접 선택(Decision Making)을 내리게 하고, 
#       그 결과(게이지 변화)를 실시간으로 보여주어 인과관계를 스스로 체득하게 함 (Simulation Learning).
# --------------------------------------------------------------------------
def render_tab3():
    st.title("⏳ 3단계: 갑신정변, 운명의 3일 시뮬레이터")
    
    # AI 윤리 교육: 생성형 AI의 결과물이 역사적 사실과 다를 수 있음을(Hallucination) 경고
    st.warning("""
    ...
    # [상태창] 게이미피케이션 요소: 성공 확률 등의 추상적 개념을 수치화하여 시각적으로 보여줌
    ...
    with col_main:
        # 타임라인 슬라이더: 시간의 흐름(3일천하)에 따라 사건 전개를 구조화함
        ...
        if timeline == "1일차: 거사 (12.4)":
            ...
            with col1:
                ...
                # 심화 학습 자료 제공 (Scaffolding): 원하는 학생만 추가 정보를 보도록 하여 수준별 학습 지원
            ...
            with col2:
                ...
                **[상황]** 우정총국 개국 축하연에서 불길이 솟아올랐습니다! 혼란을 틈타 김옥균과 급진개화파는 계획대로 민씨 정권을 몰아내고 고종 임금님을 안전한 곳으로 모셨습니다.
                ...
                # 선택지: 실제 역사(외세 의존) vs 가상 역사(자주 국방) 비교
                col_b1, col_b2 = st.columns(2)
                if col_b1.button("🅰️ 일본군에게 부탁한다 (실제 역사)"):
                    st.session_state['day1_choice'] = 'A'
                    # 실제 역사를 선택했을 때: 외세 의존으로 인한 부정적 결과(수치 하락) 피드백
            ...
            # 추가 이미지 뷰어 (AI 생성 이미지임을 명시하여 미디어 리터러시 교육)
            if st.session_state['show_more_day1']:
                ...
                st.caption("※ 위 이미지는 AI가 생성한 이미지입니다. 현실과 다를 수 있음에 주의하세요.")
            ...
            with col2:
                ...
                **[상황]** 약 1,500명의 청나라 군대가 몰려왔습니다! 우리가 믿었던 일본군은 불리해지자 슬금슬금 도망갈 준비를 합니다.
                ...
                if st.session_state['day3_choice'] is not None:
                    ...
                    # 가상 역사 (이전 선택에 따라 결과가 갈림)
                    else:
                        if final_score >= 70:
                            st.balloons() # 성공 시 풍선 효과로 성취감 부여
...
# --------------------------------------------------------------------------
# [페이지 4] 탭 4: 역사적 상상력 (평가 및 마무리)
# 목적: 수업을 정리하며 학생이 자신의 생각을 글로 표현하게 하여(Evaluation),
#       단순한 체험을 넘어 내면화된 지식으로 남도록 함.
# --------------------------------------------------------------------------
def render_tab4():
...
# --------------------------------------------------------------------------
# [메인 로직] 페이지 라우팅 (Routing)
# 목적: session_state에 저장된 current_page 값에 따라 적절한 함수를 호출하여
#       단일 페이지 앱(SPA)처럼 부드럽고 끊김 없는 화면 전환을 구현함.
# --------------------------------------------------------------------------
